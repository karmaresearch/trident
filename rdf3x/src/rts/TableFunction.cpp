...
using namespace std;
//---------------------------------------------------------------------------
TableFunction::TableFunction(Operator* input,Runtime& runtime,uint64_t id,const string& name,const vector<FunctionArgument>& inputArgs,const vector<Register*>& outputVars,double expectedOutputCardinality)
   ...
   // Request new table values
{
   ...
   for (uint64_t index=0;index<inputArgs.size();index++) {
      cout << " ";
      if (inputArgs[index].reg) {
         uint64_t v=inputArgs[index].reg->value;
         if (!~v) {
            cout << "NULL";
         } else {
            ...
            if (runtime.hasTemporaryDictionary()) {
               ...
               ok=runtime.getDatabase().lookupById(v,start,stop,type,subType);
            }
            if (!ok) {
               cout << "NULL";
            } else {
                  ...
                  switch (type) {
                     ...
                     case Type::CustomLanguage:
                        if (runtime.hasTemporaryDictionary()) {
                           ...
                           ok=runtime.getDatabase().lookupById(subType,start,stop,type,subType);
                        }
                        if (ok) {
                           cout << "@";
                           escapeOutput(start,stop);
                     ...
                     case Type::Date: cout << "\"^^<http://www.w3.org/2001/XMLSchema#date>"; break;
                  }
               }
            }
         }
   ...
   while (!done) {
      ...
      DBLayer& dict=runtime.getDatabase();
      for (vector<string>::const_iterator iter=line.begin(),limit=line.end();iter!=limit;++iter) {
         ...
         if (s.length()<2) {
            ...
            uint64_t iter=1,limit=s.length();
            for (;iter<limit;++iter)
               if (s[iter]=='\\') ++iter; else
               if (s[iter]=='"') break;
            v=s.substr(1,iter-1);
...
//---------------------------------------------------------------------------
uint64_t TableFunction::next()
   // Produce the next tuple
{
   while (true) {
      // More data?
      if (tableIter!=tableLimit) {
         for (uint64_t index=0,limit=outputVars.size();index<limit;index++)
